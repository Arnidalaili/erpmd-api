...
class AuthController extends Controller
{
    public function token(Request $request)
    {
        ...
        if (Auth::attempt($credentials)) {
            ...
            $roleUserId = DB::table("user")->select(
                'userrole.role_id',
                'user.id'
            )
                ->where('user.user', $request->user)
                ->leftJoin(DB::raw("userrole"), 'userrole.user_id', 'user.id')
                ->first();

            // return $roleUserId;

            $roleUser = DB::table("role")->select(
                'rolename',
            )
                ->where('id', $roleUserId->role_id)
                ->first();

            return response([
                'user' => $user,
                'access_token' => $user->createToken('Access Token')->accessToken,
                'role_id' => $roleUser->rolename
            ]);
        } else {
            return response([
