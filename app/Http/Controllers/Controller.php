...
class Controller extends BaseController
{
    ...
    public function toExcel(string $Laporan, array $data, array $columns)
    {
        ...
        foreach ($data as $dataIndex => $row) {
            ...
            foreach ($columns as $columnsIndex => $column) {
                $sheet
                    ->setCellValue($alphabets[$columnsIndex] . $tableHeaderRow, $column['label'] ?? $columnsIndex + 1)
                    ->getStyle($alphabets[$columnsIndex] . $tableHeaderRow)
                    ->getFont()
                    ->setSize(10)
                    ->setBold(true);
                if (
                    ...
                    if (isset($row[$column['index']])) {
                        if (substr($row[$column['index']], 0, 4) == '1900') {
                            ...
                            $sheet->getStyle($alphabets[$columnsIndex] . $startRow)->getFont()->setSize(10);
                        }
                    } else {
                        $value = $dataIndex + 1;
                    }
