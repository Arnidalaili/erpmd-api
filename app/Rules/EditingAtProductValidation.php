...
class EditingAtProductValidation implements Rule
{
    /**
    ...
    public function passes($attribute, $value)
    {
        ...
        if ($btn == 'EDIT') {
            ...
            $this->editingby = $product->editingby;
            $user = auth('api')->user()->name;
            $editingat = new DateTime(date('Y-m-d H:i:s', strtotime($product->editingat)));
            $diffNow = $editingat->diff(new DateTime(date('Y-m-d H:i:s')));
            // cek user
            if ($product->editingby != '' && $product->editingby != $user) {
