...
class EditingAtPerkiraanValidation implements Rule
{
    /**
    ...
    {
        $perkiraan = Perkiraan::find(request()->id);
        $btn = request()->btn;
        ...
        if ($btn == 'EDIT') {
            ...
            $this->editingby = $perkiraan->editingby;
            $user = auth('api')->user()->name;
            $editingat = new DateTime(date('Y-m-d H:i:s', strtotime($perkiraan->editingat)));
            $diffNow = $editingat->diff(new DateTime(date('Y-m-d H:i:s')));
            // cek user
            if ($perkiraan->editingby != '' && $perkiraan->editingby != $user) {
