...
class EditingAtArmadaValidation implements Rule
{
    /**
    ...
    public function passes($attribute, $value)
    {
        ...
        if ($btn == 'EDIT') {
            ...
            $this->editingby = $armada->editingby;
            $user = auth('api')->user()->name;
            $editingat = new DateTime(date('Y-m-d H:i:s', strtotime($armada->editingat)));
            $diffNow = $editingat->diff(new DateTime(date('Y-m-d H:i:s')));
            // cek user
            if ($armada->editingby != '' && $armada->editingby != $user) {
