...
class EditingAtGroupCustomerValidation implements Rule
{
    /**
    ...
    {
        $groupCustomer = GroupCustomer::find(request()->id);
        $btn = request()->btn;
        ...
        if ($btn == 'EDIT') {
            ...
            $this->editingby = $groupCustomer->editingby;
            $user = auth('api')->user()->name;
            $editingat = new DateTime(date('Y-m-d H:i:s', strtotime($groupCustomer->editingat)));
            $diffNow = $editingat->diff(new DateTime(date('Y-m-d H:i:s')));
            // cek user
            if ($groupCustomer->editingby != '' && $groupCustomer->editingby != $user) {
