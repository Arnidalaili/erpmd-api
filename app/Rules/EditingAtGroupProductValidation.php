...
class EditingAtGroupProductValidation implements Rule
{
    /**
    ...
    {
        $groupProduct = GroupProduct::find(request()->id);
        $btn = request()->btn;
        ...
        if ($btn == 'EDIT') {
            ...
            $this->editingby = $groupProduct->editingby;
            $user = auth('api')->user()->name;
            $editingat = new DateTime(date('Y-m-d H:i:s', strtotime($groupProduct->editingat)));
            $diffNow = $editingat->diff(new DateTime(date('Y-m-d H:i:s')));
            // cek user
            if ($groupProduct->editingby != '' && $groupProduct->editingby != $user) {
