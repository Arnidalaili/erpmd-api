...
class EditingAtBankValidation implements Rule
{
    /**
    ...
    public function passes($attribute, $value)
    {
        ...
        if ($btn == 'EDIT') {
            ...
            $this->editingby = $bank->editingby;
            $user = auth('api')->user()->name;
            $editingat = new DateTime(date('Y-m-d H:i:s', strtotime($bank->editingat)));
            $diffNow = $editingat->diff(new DateTime(date('Y-m-d H:i:s')));
            // cek user
            if ($bank->editingby != '' && $bank->editingby != $user) {
