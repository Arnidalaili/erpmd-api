...
class EditingAtAlatBayarValidation implements Rule
{
    /**
    ...
    {
        $alatBayar = AlatBayar::find(request()->id);
        $btn = request()->btn;
        ...
        if ($btn == 'EDIT') {
            ...
            $this->editingby = $alatBayar->editingby;
            $user = auth('api')->user()->name;
            $editingat = new DateTime(date('Y-m-d H:i:s', strtotime($alatBayar->editingat)));
            $diffNow = $editingat->diff(new DateTime(date('Y-m-d H:i:s')));
            // cek user
            if ($alatBayar->editingby != '' && $alatBayar->editingby != $user) {
